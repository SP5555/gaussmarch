...
struct vec2f {
    ...
    vec2f(double x, double y) : x((float)x), y((float)y) {}
};
...
struct vec3f {
    ...
    vec3f operator-(const vec3f &o) const { return {x-o.x, y-o.y, z-o.z}; }
    friend vec3f operator*(float a, const vec3f &v) { return {a*v.x, a*v.y, a*v.z}; }
    friend vec3f operator*(const vec3f &v, float a) { return {a*v.x, a*v.y, a*v.z}; }
};
